...
Let $n_{\text{treat}}$ be the number of total treatments (in our case 5, including the control), and let $n_{\text{stage1}}$ and $n_{\text{stage2}}$ be the numbers of patients allocated to each treatment in stage 1 and stage 2 of the trial, respectively. Let $k_1$ and $k_2$ denote the number of treatments in stage 1 and stage 2, respectively. Within one stage, the same number of patients are allocated to all different treatments. Then $n_{\text{treat}}$ can be calculated as
\begin{equation}
  \label{eq:ntreat}
  n_{\text{treat}} = k_1 \cdot n_{\text{stage1}} + k_2 \cdot n_{\text{stage2}},
\end{equation}
In our scenario we have $k_1 = 5$ and $k_2 \in \{2, \ldots, 5\}$, where $k_2$ depends on the result of the treatment selection strategy in stage 1.

Our main optimization goal is to maximize the statistical power $y$ depending on the trial design, where $k_2$ depends on the results of the first stage. 
The power $y$ can be formulated as a black-box as follows:
\begin{align}
  \label{eq:bbox}
...
Given the number of treatments $k_1$ and $k_2$ and the total number of patients $n_{\text{treat}}$, optimizing the power with respect to $n_{\text{stage1}}$ and $n_{\text{stage2}}$ is equivalent to optimizing with respect to $r$, since
\begin{align}
  \label{eq:stagec}
  %\begin{split}
  n_{\text{stage1}} & = 
...
These formulas give the treatments per arm and per stage if $k_2$ is known. 
However, this is only the case for selection strategies 1, 2, and 3.
For the selection strategies \emph{threshold} and \emph{epsilon}, the number of arms in the second stage ($k_2$) depends on the choice of the \emph{thresh} or \emph{eps} parameter.
...
number of simulation iterations (\texttt{nsim}~$= 1000$), correlation between early and final outcomes  (\texttt{corr}~$= 0.4$), test level (\texttt{level}~$=0.025$), and (TODO Not sure what that means) (\texttt{ptest}~$=(3,4)$). %jeweils Elementarhypothese 3, 4 verwerfen, gucken wir gar nicht drauf

\paragraph{Exhaustive Search}
For each effect set (see Table~\ref{tab:table_effect_names}) we conduct an exhaustive grid search with a resolution of 25 points per real-valued dimension for each selection strategy.
This results in total $4 \cdot 25 + 2 \cdot 25 \cdot 25 = 1350$ design configurations that are evaluated for each effect set.
Additionally the grid is evaluated with 10 stochastic repliactions to obtain an estimation of the variance of the simulation.

The best point of one grid is determined by selecting the configuration that let to the best outcome.
If we would report this outcome, we will likely be overoptimistic, as the chances are high that the noise contributed to the good outcome.
To be able to report an unbiased performance we take the best found configuration and look up the results in the remaining 9 stochastic replications of the grid evaluations, similar to a cross-validation in machine learning applications.
This eliminates the risk of only reporting outcomes that are optimal by chance.

\paragraph{Application of MBO}
Before the application of MBO, we have to define how the surrogate model that predicts the outcome of $f$ for unknown values of $\theta$ is generated.
As the regression model for the surrogate we chose Kriging (also called Gaussian process regression) and use the implementation in the R-package \texttt{DiceKriging}~\citep{roustant_dicekriging_2012}, because it is well known to work well for fairly low dimensional search spaces.
...
Kringing expects a numerical input, but our search space contains categorical parameters and is even hierarchical.
...
Furthermore, our search space is hierarchical because \emph{Epsilon} and \emph{Threshold} are only active for the respective \emph{selection strategy}.
...
To set the inactive value to a high value outside of the original range ensures that within the original range the Kriging-model does not or only marginally gets affected by the inactive values.
Note, that this trick only works because we did not scale the input variables to $[0,1]$ and because the estimated covariance by the Mattern$\frac{5}{2}$ kernel is approximately zero for distances greater than $4$.
With the above steps we ensured that the input for the Kriging is purely numeric.

As an acquisition function we chose the $\operatorname{AEI}$ as explained in Section~\ref{sec:model_based_optimization}.
...
At the end of the optimization procedure, the best point is chosen according to the best prediction obtained on the set of evaluated poins as explained in Section~\ref{ssec:best_point}.
The best selected point is then evaluated $10$ times to allow us to report an an unbiased estimate of the true performance of the optimization result.
