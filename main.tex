...
\tikzset{nearnodes/.style={draw, node distance=0cm,}}
\begin{tikzpicture}[auto,node distance=0.3cm,align=center,text width = 3cm, font=\scriptsize]
  \node[draw, ellipse, minimum height=1cm, text width=3cm] (ClinicalScenarios) at (0,0) {
    \textbf{Clinical scenarios}\\
    Simulation Studies};
  \node[name=block, rectangle split, rectangle split parts=2, nearnodes, above left = of ClinicalScenarios] (HistoricalDataPoints)
  { \begin{itemize}[leftmargin=.3cm]
      \item Correlation between early interim and late disability outcomes.
    \end{itemize}
    \nodepart{second} Combination of options
  };
  \node[name=block, rectangle split, rectangle split parts=2, nearnodes, left = of HistoricalDataPoints] (UnknownPoints)
  { \begin{itemize}[leftmargin=.3cm]
  ...
  %\node[nearnodes, inner xsep=1em, inner ysep=1em, above left = of ClinicalScenarios, fit = (HistoricalDataPoints) (UnknownPoints)] (boxDiseaseSpecific) {};
  \node[draw, above = of HistoricalDataPoints, text width = 6cm] (DiseaseSpecific) {\textbf{Disease-specific features}};
  %\node[draw, above right = of ClinicalScenarios] (DesignOptions) {Design options};
  \node[name=block, rectangle split, rectangle split parts=2, nearnodes, above right = of ClinicalScenarios] (CombinationPoints)
  { \begin{itemize}[leftmargin=.3cm]
  ...
  \node[name=block, rectangle split, rectangle split parts=2, nearnodes, left = of CombinationPoints] (ConstrainedPoints)
  { \begin{itemize}[leftmargin=.3cm]
  ...
  \node[draw, above = of ConstrainedPoints,text width = 6cm] (DesignOptions) {\textbf{Disease-specific features}};
  \node[draw, below = of ClinicalScenarios] (DesignPerformance) {
    \textbf{Design performance} \\ e.g.\ statistical power};
  \node[draw, right = of DesignPerformance] (Evaluation) {
